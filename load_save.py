...
class Load_Save:
    ...
    def load_game_parm(self,fname):        
        try:
                fp = open(fname)
        except  OSError as e:#FileNotFoundError as
                if e.errno == errno.EACCES:
                        gv.gui.set_status_bar_msg(
                        "Error  loading game - no permissions for file")
                if e.errno == errno.ENOENT:
                        gv.gui.set_status_bar_msg(
                        "Error  loading game - file doesn't exist")
                if e.errno == errno.EROFS:
                        gv.gui.set_status_bar_msg(
                        "Error  loading game - file readonly")
